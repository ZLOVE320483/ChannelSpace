...
public class CustomerFromIndependentFragment extends BaseFragment implements OnClickListener, OnItemClickListener, PullableViewListener {
    ...
    public void onClick(View v) {
        if (v == btnClassify) {
            ...
            intent.putExtra(IntentKey.INTENT_KEY_CUSTOMER_TYPE, "1");
    ...
    class GetClientListHandler extends HttpResponseHandlerFragment<CustomerFromIndependentFragment> {

        private LOAD_ACTION action;
        
        public GetClientListHandler(CustomerFromIndependentFragment context, LOAD_ACTION action) {
            super(context);
            this.action = action;
        }
    ...
    public void onLoadMore() {
        ClientHttpRequest.getClientListRequest(keyWord, categoryId, houseType, propertyType, String.valueOf(pageIndex), "10", status,
            ChannelCookie.getInstance().getCurrentHouseId(), "1", isDisabled, new GetClientListHandler(this, LOAD_ACTION.LOADERMORE));
    }
    
    private void getClientList() {
        ClientHttpRequest.getClientListRequest(keyWord, categoryId, houseType, propertyType, "0", "10", status, 
            ChannelCookie.getInstance().getCurrentHouseId(), "1", isDisabled, new GetClientListHandler(this, LOAD_ACTION.ONREFRESH));
    }
