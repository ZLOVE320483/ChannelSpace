...
public class DoubleDirectSeekBar extends View {
    ...
    /*
     * private static final int[] PRESSED_STATE_SET = { android.R.attr.state_focused, android.R.attr.state_pressed,
    ...
    private int initLow = 0;
    private int initHigh = 1000;
    
    public void setInitLow(int initLow) {
        this.initLow = initLow;
    }
    
    public void setInitHigh(int initHigh) {
        this.initHigh = initHigh;
    }

    public DoubleDirectSeekBar(Context context) {
        this(context, null);
    }

    public DoubleDirectSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DoubleDirectSeekBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        mThumbHigh = resources.getDrawable(R.drawable.seekbar_thumb);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mOffsetLow = formatDouble((defaultScreenLow + initLow) / getDifference() * (mDistance)) + mThumbWidth / 2;
        mOffsetHigh = formatDouble((defaultScreenHigh - (1000 - initHigh)) / getDifference() * (mDistance)) + mThumbWidth / 2;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        text_Paint.setColor(ApplicationUtil.getApplicationContext().getResources().getColor(R.color.common_text_green));
        ...
        // 前滑块
        mThumbLow.setBounds((int) (mOffsetLow - mThumbWidth / 2), mThumbMarginTop, (int) (mOffsetLow + mThumbWidth / 2), mThumbHeight
            + mThumbMarginTop);
        mThumbLow.draw(canvas);

        // 后滑块
        mThumbHigh.setBounds((int) (mOffsetHigh - mThumbWidth / 2), mThumbMarginTop, (int) (mOffsetHigh + mThumbWidth / 2), mThumbHeight
            + mThumbMarginTop);
        mThumbHigh.draw(canvas);

        double progressLow = formatDouble((mOffsetLow - mThumbWidth / 2) * getDifference() / mDistance);
        double progressHigh = formatDouble((mOffsetHigh - mThumbWidth / 2) * getDifference() / mDistance);
        
        int low = ((int) progressLow / 50) * 50;
        int high = ((int) progressHigh / 50) * 50;
        
         if (mOffsetHigh - mOffsetLow > 100) {
        	 canvas.drawText(String.valueOf(low), (int) mOffsetLow - 2 - 2, 25, text_Paint);
             if (progressHigh == defaultScreenHigh) {
                 canvas.drawText("不限", (int) mOffsetHigh - 2, 25, text_Paint);
             } else {
                 canvas.drawText(String.valueOf(high), (int) mOffsetHigh - 2, 25, text_Paint);
             }
         } else {
        	 canvas.drawText(String.valueOf(low), (int) mOffsetLow - 25 - 2, 25, text_Paint);
             if (progressHigh == defaultScreenHigh) {
                 canvas.drawText("不限", (int) mOffsetHigh - 2, 25, text_Paint);
             } else {
                 canvas.drawText(String.valueOf(high), (int) mOffsetHigh + 25, 25, text_Paint);
             }
         }
    ...
    public int getAreaFlag(MotionEvent e) {
        ...
        if (e.getY() >= top && e.getY() <= bottom && e.getX() >= (mOffsetLow - mThumbWidth / 2) && e.getX() <= mOffsetLow + mThumbWidth / 2) {
            ...
            && e.getY() <= bottom
            && ((e.getX() >= 0 && e.getX() < (mOffsetLow - mThumbWidth / 2)) || ((e.getX() > (mOffsetLow + mThumbWidth / 2)) && e.getX() <= ((double) mOffsetHigh + mOffsetLow) / 2))) {
            return CLICK_IN_LOW_AREA;
        } else if (e.getY() >= top
            && e.getY() <= bottom
            && (((e.getX() > ((double) mOffsetHigh + mOffsetLow) / 2) && e.getX() < (mOffsetHigh - mThumbWidth / 2)) || (e.getX() > (mOffsetHigh + mThumbWidth / 2) && e.getX() <= mScollBarWidth))) {
    ...
    public void setProgressLow(double progressLow) {
        ...
        mOffsetLow = formatDouble(progressLow / getDifference() * (mDistance)) + mThumbWidth / 2;
        isEdit = true;
        refresh();
    }
    ...
    public void setProgressHigh(double progressHigh) {
        ...
        mOffsetHigh = formatDouble(progressHigh / getDifference() * (mDistance)) + mThumbWidth / 2;
        isEdit = true;
        refresh();
    }
    ...
    /*
     * private int formatInt(double value) { BigDecimal bd = new BigDecimal(value); BigDecimal bd1 = bd.setScale(0,
    ...
    private int getDifference() {
        return (int) (defaultScreenHigh - defaultScreenLow);
    }
}
